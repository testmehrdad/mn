...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/testmehrdad/mn/master/install_en.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/testmehrdad/mn/raw/master/x-ui_en.sh
    if [[ $? != 0 ]]; then
        echo ""
