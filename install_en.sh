...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        last_version=$(curl -Ls "https://api.github.com/repos/testmehrdad/mn/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-${arch}-english.tar.gz https://github.com/testmehrdad/mn/releases/download/${last_version}/x-ui-linux-${arch}-english.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/testmehrdad/mn/releases/download/${last_version}/x-ui-linux-${arch}-english.tar.gz"
    ...
    wget --no-check-certificate -O /usr/bin/x-ui https://raw.githubusercontent.com/testmehrdad/mn/main/x-ui_en.sh
